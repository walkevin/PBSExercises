	...
	vmath::mat4 RotatingCamera::getProjectionViewMatrix(double azimuth, double polar, double distance){
		...
		return vmath::perspective(60.0f, aspectRatio, 1.0f, 500.0f) *
				vmath::lookat(
						...
						//vmath::vec3(std::cos(azimuth) * std::sin(polar) , std::cos(polar+0.5*M_PI) , std::sin(azimuth) * std::sin(polar) ),
						vmath::vec3(0.0f, 1.0f, 0.0f)
						//vmath::vec3(std::cos(azimuth) * std::sin(polar) , std::cos(polar+M_PI) , std::sin(azimuth) * std::sin(polar) )
						);
	}
...
//		glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
	...
	void RotatingCamera::createVBO(){
		...
		{
			//Back
			{ { -0.4f, -0.4f, -0.5f, 1.0f }, { 1.0f, 1.0f, 1.0f, 1.0f } }, // 0
			{ { -0.4f, 0.4f, -0.5f, 1.0f }, { 0.0f, 1.0f, 1.0f, 1.0f } }, // 1
			{ { 0.4f, 0.4f, -0.5f, 1.0f }, { 0.0f, 0.0f, 1.0f, 1.0f } }, // 2
			{ { 0.4f, -0.4f, -0.5f, 1.0f }, { 1.0f, 1.0f, 1.0f, 1.0f } }, //3
